...
export class PlayersService {
  ...
  async search(dto: SearchPlayersDto, userRole?: Role) {
    ...
    if (teamId) {
      // ✅ Team tablosundan hem takım adını hem bağlı ligi çek
      const team = await this.prisma.team.findUnique({
        where: { id: teamId },
        include: { league: true }, // League: { name, season }
      });
      ...
      if (team) {
        ...
        // ✅ KRİTİK: Lig adıyla da filtrele — U14/U17/U19 karışmasın
        // Player.league = "U14 Gelişim Ligi" (düz metin)
        // team.league.name = "U14 Gelişim Ligi"
        // → sadece aynı yaş kategorisindeki oyuncular gelir
        where.league = { contains: team.league.name, mode: 'insensitive' };
      } else {
        // Geçersiz teamId — boş dön
        return {
          data: [],
      ...
      // teamId yoksa manuel club/league parametreleri
