...
export class ClaimsService {
  ...
  async updatePosition(userId: string, playerId: string, position: string) {
    const user = await this.prisma.user.findUnique({ where: { id: userId } });
    if (!user || user.role !== 'premium') throw new ForbiddenException('Premium üyelik gerekiyor.');
    if (user.claimedPlayerId !== playerId || user.verificationStatus !== 'approved') {
      throw new ForbiddenException('Bu profili düzenleme yetkiniz yok.');
    }
    const validPositions = ['Kaleci', 'Defans', 'Orta Saha', 'Forvet'];
    if (!validPositions.includes(position)) throw new BadRequestException('Geçersiz mevki.');
    return this.prisma.player.update({ where: { id: playerId }, data: { position } });
  }
