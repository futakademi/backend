...
export class PlayersService {
  ...
  async findOne(id: string, userRole?: Role) {
    ...
    // Sıradaki maç: oyuncunun takımının yaklaşan maçını bul
    const nextMatch = await this.prisma.match.findFirst({
      where: {
        homeScore: null, // oynanmamış maçlar
        OR: [
  ...
  private sanitizePlayer(player: any, userRole?: Role) {
    // customData herkese açık — oyuncu premium alıp doldurmuşsa herkes görür.
    // Sadece hassas alanlar (örn: gelecekte eklenirse) burada filtrelenir.
    return player;
  }
}
